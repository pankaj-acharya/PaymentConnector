    ...
    namespace Commerce.HardwareStation.EFRSample
    {
        ...
        public class EFRHandler : INamedRequestHandler
        {
            ...
            private Response SubmitDocument(SubmitDocumentFiscalDeviceRequest request)
            {
                ...
                try
                {
                    using (var timeoutCancellationTokenSource = new CancellationTokenSource())
                    {
                        ...
                        var task = SalesTransactionRequestController.RegisterAsync(request.Document,
                            ConfigurationController.GetEndpointAddress(request.PeripheralInfo.DeviceProperties), timeoutCancellationTokenSource.Token);
                        if (task.Wait(timeoutValue > 0 ? timeoutValue : System.Threading.Timeout.Infinite))
                        {
                            string responseFromService = task.Result;
                            response = SalesTransactionRequestController.ParseResponse<SubmitDocumentFiscalDeviceResponse>(responseFromService);
                        }
                        else
                        {
            ...
            private Response IsReady(IsReadyFiscalDeviceRequest request)
            {
                ...
                {
                    RetailLogger.Log.HardwareStationFiscalPeripheralFailure("Cannot check fiscal device state.", ex);
                }
