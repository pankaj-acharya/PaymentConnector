namespace Contoso
{
    namespace Commerce.HardwareStation.PaymentSample
    {
        ...
        public class PaymentDeviceSample : INamedRequestHandler//Somitra Comment out,IPaymentDevice
        {
            #region FileLog
            ...
            public PaymentDeviceSample()
            {
                ...
                WriteLine("Entered constructor for method: PaymentDeviceSample ", true);
            }
            ...
            {
                WriteLine("Entered method: Execute", true);
            ...
            {
                WriteLine("Entered method: Open()",true);
                //if (request == null)
                //{
            ...
            {
                WriteLine("Entered method: BeginTransaction()", true);
            ...
            {
                WriteLine("Entered method: UpdateLineItems", true);
                //if (request == null)
                //{
                //    throw new ArgumentNullException("request");
                //}
            ...
            {
                WriteLine("Entered method: AuthorizePayment", true);
                if (request == null)
                {
                    throw new ArgumentNullException("request");
                }
            ...
            public CapturePaymentTerminalDeviceResponse CapturePayment(CapturePaymentTerminalDeviceRequest request)
            {
                ...
                CapturePaymentTerminalDeviceResponse _returnVal = new CapturePaymentTerminalDeviceResponse(new PaymentInfo());
                return _returnVal;
            ...
            public async Task<PaymentInfo> AuthorizePaymentAsync(decimal amount, string currency, string voiceAuthorization, bool isManualEntry, ExtensionTransaction extensionTransactionProperties)
            {
                try
                {
                    WriteLine("Entered method: AuthorizePaymentAsync", true);
                    PaymentInfo paymentInfo = new PaymentInfo();
                    //Read the  test XML as string
                    string myXmlString = BuildSampleCardRequestXML();

                    // Create the XmlDocument.
                    //XmlDocument doc = new XmlDocument();
                    //doc.LoadXml(myXmlString);

                    var response = postXMLData("CardService/", myXmlString);

                    return paymentInfo;
                }
                catch (Exception ex)
                {
                    WriteLine($"Exception in : AuthorizePaymentAsync .InnerException :{ex.InnerException} ", true);
            ...
            public ConnectorConfig ReadConnectorConfiguration()
            {
                ...
                {
                    WriteLine($"Exception in : ReadConnectorConfiguration() .InnerException :{ex.InnerException} ", true);
                }
                return config;
            }
            ...
            public string postXMLData(string destinationPath, string requestXml)
            {
                try
                {
                    var _config = ReadConnectorConfiguration();
                    string urlwithPortAndPath = _config.EndPointIp + destinationPath;
                ...
                catch (Exception ex)
                {
                    ...
                    WriteLine($"Exception in : postXMLData() .InnerException :{ex.InnerException} ", true);
                }
                return null;
            }
            ...
            private void WriteLine(string text, bool append)
            {
                try
                {
                    using (System.IO.StreamWriter writer = new System.IO.StreamWriter(logFilename, append, System.Text.Encoding.UTF8))
                    {
                        ...
                        {
                            writer.WriteLine(System.DateTime.Now.ToString(datetimeFormat) + text);
                        }
                    }
                }
                catch
